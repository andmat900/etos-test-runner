...
class TestRunner:
    ...
    def run_tests(self, workspace) -> bool:
    ...
    def outcome(self, result, executed, description, test_framework_exit_code):
        ...
        custom_verdict = None
        verdict_rule_file = "verdict_rule.json"
        if verdict_rule_file is not None:
            rules = [
                {
                    "description": "Test framework error, no artifacts created",
                    "condition": {
            ...
            self.logger.info("Matched verdict rule: %s", custom_verdict)
